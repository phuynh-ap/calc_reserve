...
#   - Ensure the lMachType list has all machine buses you'd like to check
#     for reserve.
...
# - Machine ID (str)
# - Machine type (str)
lMachType = [   ["88S-LINGAN_1",    199001, "1", "Thermal"],
...
def runmain():
    ...
    for mVal in lMachType:
        if( (mVal[3] == "Thermal") or (mVal[3] == "CT") or (mVal[3] == "Hydro") ):
            # Get machine status
            iErr, mStat = psspy.macint( ibus    = mVal[1],
                                        id      = mVal[2],
                                        string  = "STATUS"
                                        )
            # Get machine bus status
            iErr, bStat = psspy.busint( ibus    = mVal[1],
                                        string  = "TYPE"
                                        )

            if( (mStat) and (bStat != 4) ):
                # Get machine pgen
                iErr, pGen = psspy.macdat(  ibus    = mVal[1],
                                            id      = mVal[2],
                                            string  = "P"
                                            )
                # Get machine pmax
                iErr, pMax = psspy.macdat(  ibus    = mVal[1],
                                            id      = mVal[2],
                                            string  = "PMAX"
                                            )
                ...
                print("{:<10s}".format(str(mVal[1]) + mVal[2]) + "{:>8.2f}".format(pGen) + "{:>8.2f}".format(pMax) + "{:>8.2f}".format(pMax - pGen) + " " + mVal[0])
